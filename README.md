...
* Adds a larger FOV range
* Adds the ability to run at higher than native render scales (supersampling)
* Adds antialiasing options including the option to select between FXAA, SMAA, TAA or None
* Allows for granular choices over resolution scale, shadow settings, upscaling and LOD quality
* Settings can also be turned down past the lowest Big Walk settings for increased performance
...
* LOD Quality is increased to 4.0 so far objects are visible
* Render Scale is at 1.4
* AA is disabled, the render scale should fix jagged foliage etc - if it's not enough for you SMAA is the least aggressive in this game
* **This can be configured entirely by you**
...
* Configuration can be found at: `(steam directory)\BIG WALK\BepInEx\config\com.github.glarmer.Big_Visuals.cfg`
...
2.) Extract the contents of that zip into your game directory (default: `C:\Program Files (x86)\Steam\steamapps\common\BIG WALK`) resulting in a folder that has the following files: <br>
...
4.) Navigate to `...\BIG WALK\BepInEx\plugins`, Download and extract the `com.github.glarmer.Big_Visuals.dll` from "Manual Download" on Thunderstore into this folder. <br>
5.) Run the game <br>
...
* Please search the BIG WALK Steam discussions if you face bugs as they are possibly caused by the game and not the mod. If you continue to experience issues then please reach out via the GitHub issues.
* These options will effect performance in some way, high values of Render Scale in particular can reduce FPS.

The only official pages for this mod are as follows:

* Thunderstore: https://thunderstore.io/
* GitHub: https://github.com/glarmer/

Any other site is not run by me and may contain malware. Please make sure to download from an official source.

## Help

* You can find your BIG WALK folder by **right clicking the game in your Steam library** then > **Manage** > **Browse Local Files**
* You can get help from myself via my [Discord server](https://discord.gg/cjzWT37abR)!
* Please report issues to the mod's GitHub, alternatively I will periodically check the Steam discussions.
